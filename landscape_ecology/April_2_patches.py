...
def makeScratchfile(oldfile,ext):
    
    """ Renames the passed file relative to the current time and puts in the scratch path,  """
 
    try:
        oldfilePath= os.path.join(watchFolder,(oldfile+ext))
        out_time=getTime("time")
        outFileName= oldfile+ "_"+out_time+ext
        outputpath= os.path.join(scratchPath,outFileName)
        print (outputpath)
        os.rename(oldfilePath,outputpath)
...
def particle(obj,specieType,count,specieSize=1):
    ...
    def treePatchFill(self,patch):
                ...
                #time=getTime("sec")
                    ...
                    try:
                        ...
                        obj=bpy.context.scene.objects.active.name
                        specieType= obj.split("_")[1]
                        patchIndex=obj.split("_")[2]
                        
                        # calculate number of species based on density and apply particle system# 
                        selectOnly(obj)
                        count=calcArea(obj)/1000
                        changeMat(obj,"Transparent")
                        particle(obj,specieType,count)

                        
                        makeScratchfile(obj,".shp")
                        makeScratchfile(obj,".shx")
                        makeScratchfile(obj,".dbf")
                        makeScratchfile(obj,".prj")
...
#                            print ("point file found but not imported") 
                    ...
                    if self.emptyTree in fileList:                                           
                        remove("patch_")
                        makeScratchfile(os.path.join(watchFolder,"empty.txt") ,"_empty.txt")
...
#                            
                    ...
                    for file in fileList:
                        patch=file.split(".")[0]
                        if file.startswith("patch_") and file[-4:]==".shp" and patch not in bpy.data.objects : 
                            print ("patch file was observed")
                            if os.path.exists(os.path.join(watchFolder,file[:-4]+".shx")) and (os.path.join(watchFolder,file[:-4]+".dbf")) and (os.path.join(watchFolder,file[:-4]+".prj")):
                          
                                adapt().treePatchFill(file)
