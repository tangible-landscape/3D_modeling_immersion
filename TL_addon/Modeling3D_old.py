...
class adapt:
    ...
    def vantageShp(self,vantagePath):
...
class ModalTimerOperator(bpy.types.Operator):
        ...
        def modal(self, context, event):
            ...
            if event.type == "TIMER":
                ...
                if self._timer.time_duration != self._timer_count:
                    ...
                    fileList = (os.listdir(self.paths.watchFolder))

                    if terrainFile in fileList:
                        try:
                            adapt().terrain(self.paths.terrainPath)
                            self.adaptMode = "TERRAIN"
                            print self.paths.terrainPath
                        except:
                            print ("terrain file found but not imported")

                    if waterFile in fileList:
                        adapt().waterFill(self.paths.waterPath)
                    ...
                    if trailFile in fileList:
                        adapt().trails(self.paths.trailPath)
                    ...
                    if emptyFile in fileList:
                        ...
                        makeScratchfile(self.paths.emptyPath, "text")

                    if vantageFile in fileList:
                        adapt().vantageShp(self.paths.vantagePath)
                    ...
                    for fileName in fileList:
                        if(
                            ...
                            adapt().treePatchFill(fileName, self.paths.watchFolder)
        ...
        def execute(self, context):
            ...
            for file in os.listdir(self.paths.watchFolder):
                try:
                    os.remove(os.path.join(self.paths.watchFolder, file))
                except:
